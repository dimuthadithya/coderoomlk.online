...
export default function Instructor() {
  const { name, title, image, bio, about, stats, skills, socials } =
    instructorData;
      ...
      <div className='max-w-7xl mx-auto px-4'>
        <div className='flex flex-col lg:flex-row items-center gap-16'>
          ...
          >
            ...
            {instructorData.company && (
              <div className='text-lg font-medium mb-6 flex items-center gap-2'>
                <span className='text-primary'>
                  {instructorData.company.role}
                </span>
                <span className='text-base-content/40'>at</span>
                <a
                  href={instructorData.company.url}
                  target='_blank'
                  rel='noopener noreferrer'
                  className='link link-hover text-base-content hover:text-accent font-bold'
                >
              ...
              <div className='flex items-center gap-3 mb-6 text-base-content/80 bg-base-200/50 p-3 rounded-lg w-fit'>
                ...
                <div>
                  <span className='font-bold'>
                    {instructorData.education.institute}
                  </span>
                  <span className='mx-2 opacity-40'>|</span>
                  <span className='text-sm opacity-70'>
                    {instructorData.education.batch}
                  </span>
                </div>
              </div>
            )}
            ...
            <p className='text-base-content/60 mb-8'>{about}</p>
            ...
            <div className='grid grid-cols-3 gap-4 mb-8'>
              {stats.map((stat, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className='text-center p-4 bg-base-200 rounded-2xl'
                  >
            ...
            <div className='mb-8'>
              ...
              <div className='flex flex-wrap gap-2'>
                {skills.map((skill, index) => (
                  <span
                    key={index}
                    className='badge badge-outline badge-lg p-4 hover:bg-primary hover:text-white transition-colors cursor-default'
                  >
                    {skill}
                  </span>
                ))}
