...
const CSV_URL =
  'https://docs.google.com/spreadsheets/d/e/2PACX-1vTYDTWrzNe9BvPz7bJxCQfohfTynfuflucxKHaKEsNjQkuEqFguJXFif1vu2xc_7ZrZSmwwxYF9Pqv1/pub?gid=1711080674&single=true&output=csv';
...
async function main() {
  try {
    ...
    console.log(
      `Last fetched time from state: ${new Date(lastFetchedTime).toLocaleString()}`,
    );
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (recordTime > lastFetchedTime) {
        ...
        const testimonial = {
          name: csvRow['Name (optional)'] || '',
          github: csvRow['Github Username (optional)'] || '',
    ...
    if (newRecordsCount > 0) {
      ...
      console.log(
        `Saving updated data (${existingData.length} total records) to ${OUTPUT_FILE}...`,
      );
      fs.writeFileSync(OUTPUT_FILE, JSON.stringify(existingData, null, 2));

      // Update state
      const newState = {
        lastFetchedTime: new Date(latestTimeInBatch).toISOString(),
      }; // Store as ISO string
      ...
      if (!fs.existsSync(STATE_FILE)) {
        const newState = {
          lastFetchedTime: new Date(lastFetchedTime).toISOString(),
        };
        fs.writeFileSync(STATE_FILE, JSON.stringify(newState, null, 2));
        console.log(
          `Created state file with timestamp: ${newState.lastFetchedTime}`,
        );
      }
    }
  } catch (err) {
