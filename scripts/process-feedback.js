    ...
    try {
        ...
        lines.forEach(line => {
            ...
            // Format 2: Timestamp | Feedback | Suggestions | [Optional] | Name
                ...
                if (name && text && !name.includes("Timestamp")) { // Double check
                    testimonials.push({
                        name: cleanName(name),
                        github: github,
                        text: cleanText(text),
        ...
        const uniqueTestimonials = testimonials.filter((v,i,a)=>a.findIndex(t=>(t.name===v.name && t.text.substring(0,20)===v.text.substring(0,20)))===i);

        fs.writeFileSync(JSON_FILE, JSON.stringify(uniqueTestimonials, null, 2));
        console.log(`Successfully converted ${uniqueTestimonials.length} testimonials to JSON.`);
